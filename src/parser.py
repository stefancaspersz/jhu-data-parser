...
def fix_record(record_dict):
    ...
    for key in record_dict.keys():
        if match("^\d{1,2}/\d{1,2}/\d{2}$", key):
            ...
            if key in ["lat", "long"]:
                fixed_record[key] = float(record_dict[key])
            else:
                if key == "Country/Region":
                    if record_dict[key] == "US":
                        fixed_record[key] = "United States"
                    else:
                        fixed_record[key] = record_dict[key]
                else:
                    fixed_record[key] = record_dict[key]
...
def parse_and_store(category, input_csv):
    ...
    for row in recordset:
        ...
        if record['Province/State'] == '':
            s3key = 'type=' + category + '/' + record['Country/Region'] + '.json'
        else:
